...
## Architecture & Technical Reference — 14 September 2026

**Status:** Authoritative
**Supersedes:** `architecture_flow_28Aug.md`, `architecture_flow_19Aug (1).md`, `architecture_flow (2).md`
...
**Current demo scope:** Enel S.p.A. (`CLI101`), one client, whitelisted for the presentation.
The pipeline is designed for multi-client operation and has been load-tested with two clients.
...
| `/api/metrics` | `ca_opportunity_scoring`, `client_master`, `digital_twin_signals` |
...
produced during ingestion. It is not calculated by a formula. When the extraction runs, Gemini
returns a `priority_score` value alongside other signal fields, and that value is written to
the opportunity row.
...
The score is displayed to the RM as a match-confidence indicator. It does not update as new
signals arrive; it reflects the last ingestion that wrote it. A post-demo improvement would
be to recompute it against the accumulated signal corpus.
...
│     e. Write to _MANDATE_SYNTH_CACHE and to ca_opportunity_scoring          │
└────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────────┐
...
`_DEMO_CLIENT_IDS = {"CLI101"}` in `main.py` scopes LLM synthesis to the current demo
client(s). Clients not in the set skip the Gemini call and read their DB row directly.
...
Cache entries are keyed by `client_id` and expire after 5 minutes. This eliminates repeat
Gemini calls for the same client within a demo session.
...
**Known divergence risk:** if the two paths compute values independently (e.g., savings
strings, tenor formatting), they can drift. Currently:

- React computes some values (e.g., savings) client-side.
- The PPTX builder computes the same values server-side.
- A future refactor could unify these, but for the demo they match.
...
| GET | `/api/metrics` | RM dashboard metrics (active drafts, avg time, priorities) |

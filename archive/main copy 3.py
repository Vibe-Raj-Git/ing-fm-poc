...
def get_opportunities():
    ...
    if conn:
        try:
            ...
            credit_spreads = {}
            try:
                cur.execute("SELECT tenor, spread_bps, all_in_yield_pct FROM ca.ext_credit_spreads WHERE issuer_or_rating ILIKE %s OR issuer_or_rating ILIKE %s", (f"%{tier}%" if tier else "%BBB%", f"%{name_str}%"))

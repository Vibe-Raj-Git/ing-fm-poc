...
**Supersedes:** `master_persona_18Sep.md` (18 Sep 2026), `master_persona_14Sep.md` (14 Sep 2026), `master_persona_19Aug (1).md`, `master_persona_14Aug (1).md`
...
7. **Beware long heredoc pastes.** Terminal paste of multi-line heredocs has repeatedly corrupted content (commit messages, README, patch scripts). Prefer shorter blocks, file-based writes via editor, or `python3` with a single string. Verify with `wc -l` and `grep` after every paste.
...
| `synthesize_mandate_catalyst()` | LLM synthesis anchored to `ca_opportunity_scoring`; returns **5 keys**: `why_now`, `action`, `why_now_summary`, `action_summary`, `priority_score` |
...
| `_MANDATE_SYNTH_CACHE` | In-memory TTL cache (300s), **6-tuple**: `(expiry, why_now, action, why_now_summary, action_summary, priority_score)`. Populated **only after** `conn.commit()` succeeds; popped on failure. See §7.8 |
...
| `build_pitchbook()` | Renders 11 slides with `python-pptx`. Slide 5/10 tranche table reads from `ctx["maturities"]` |
...
The prompt produces **five keys**: `why_now`, `action` (full narratives for slide 3), `why_now_summary`, `action_summary` (max 160 chars each, for slide 2), and `priority_score` (integer 0-100, weighted rubric: signal strength 40% / balance-sheet pressure 30% / market window 30%). Synthesis runs at `temperature=0.0`.
...
- Key: `client_id`
- Value: **6-tuple** `(expiry_epoch, why_now, action, why_now_summary, action_summary, priority_score)`
- TTL: 300 seconds
